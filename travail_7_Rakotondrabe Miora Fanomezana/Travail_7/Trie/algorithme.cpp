...
int triBulles(T* a, int n)
{
	...
	duree = chrono::duration_cast<chrono::nanoseconds>(end - start).count();
	return duree;
}

template<class T>
void triBullesInverser(T* a, int n) {
	for (int i = 0; i < n; i++)
		for (int j = 1; j < n - i; j++)
		{
			if (a[j - 1] < a[j]) {
